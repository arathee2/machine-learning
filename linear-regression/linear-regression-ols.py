...
class LinearRegression:
    ...
    def fit(self, X_train, y_train):
        ...
        self.__weights = np.matmul(np.matmul(np.linalg.inv(
            np.matmul(self.X_train.T, self.X_train)), self.X_train.T), self.y_train)
    ...
    def predict(self, X_test):
        ...
        assert X_test.shape[1] == self.X_train.shape[1] - \
            1, "Error! Test data must have same number of features as train data.\n Features in trianing set = {}.\n Features in test set = {}.".format(
                self.X_train.shape[1]-1, X_test.shape[1])
